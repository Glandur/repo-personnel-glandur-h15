...
<%@ page import="ca.etsmtl.gti525.devoir1.Photo"%>
<%@ page import="ca.etsmtl.gti525.devoir1.Photo"%>
<%@ page import="java.util.ArrayList"%>
<%@ page import="java.util.List"%>
<%
	List<Photo> listeImages = (ArrayList<Photo>)request.getAttribute("collection");
%>
<!DOCTYPE html>
<html>
<head>
		...
		<select name="listeImages"
			...
			<%
				for (int i = 0; i < listeImages.size(); i++) {
					Photo image = listeImages.get(i);
			%>
					<option value="<%=image.getId()%>"><%=image.getChemin()%></option>
			<%
				}
			%>
		</select>
